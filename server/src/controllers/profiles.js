...
exports.list = function (req, res) {
  ...
  Profile.paginate(query, request.getRequestOptions(req), function (err, result) {
...
exports.update = function (req, res) {
  const attrs = _.pick(req.body, "title", "about", "cover")
  Profile.findOneAndUpdate({ _id: req.params.id }, attrs, { new: true }, function (err, item) {
    if (err) return response.sendBadRequest(res, err);
...
exports.delete = function (req, res) {
  Profile.remove({ _id: req.params.id }, function (err, item) {
    if (err) return response.sendNotFound(res);
    if (!req.currentUser.canEdit(item)) return response.sendForbidden(res);
