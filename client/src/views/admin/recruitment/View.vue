        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
          <div class="card mt-8 shadow-lg rounded-md p-8">
            <div class="flex justify-between">
              ...
              <router-link :to="{ name: 'recruitment-new' }" class="
                  inline-flex
                  justify-center
                  py-2
                  px-8
                      ...
                      <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="(recruitment, idx) in recruitments" :key="idx">
                          ...
                          <td class="px-6 py-4 whitespace-nowrap">
                            <span v-if="recruitment.status" class="
                                px-2
                                inline-flex
                                text-xs
                                leading-5
                          ...
                          <td class="
                              ...
                              h-full
                            ">
                            <router-link :to="{ name: 'recruitment-edit', params: { id: recruitment._id } }">

                              <button class="p-3 mx-1 rounded-lg hover:bg-indigo-600 hover:text-white text-gray-400"
                                type="button">

                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" style="width: 1rem;">
                            ...
                            <button class="
                                ...
                                hover:bg-indigo-600 hover:text-white
                                text-gray-400
                              " type="button" @click="deleteModel(recruitment._id)">
...
export default {
  mounted() {
    ...
    axios
      .get(`${serverUrl}/admin/recruitments`, {
        headers: {
  ...
  methods: {
    deleteModel(id) {
      ...
      showConfirmPopup(function () {
        const authStore = useAuthStore();
        axios
          .delete(`${serverUrl}/admin/recruitments/${id}`, {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${authStore.token}`,
              "x-access-token": authStore.token,
    ...
    approveModel(id) {
      ...
      showConfirmPopup(
        function () {
          ...
          axios
            .put(
              `${serverUrl}/admin/recruitments/${id}/approve`,
              { status: true },
              {
                headers: {
                  "Content-Type": "application/json",
            ...
            .then(function () {
              self.recruitments = self.recruitments.map((item) =>
                item._id === id ? Object.assign(item, { status: true }) : item
              );
              createToast("Success approve recruitment", { type: "success" });
            });
        },
