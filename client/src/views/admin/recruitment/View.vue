...
export default {
  mounted() {
    axios
      .get(`${serverUrl}/recruitments`, {
        params: {
          limit: 9999,
        },
      })
      .then((res) => {
        this.recruitments = res.data["docs"];
      });
  },
  data: () => ({
    recruitments: [],
  ...
  methods: {
    ...
    approveModel(id) {
      ...
      showConfirmPopup(function () {
        const authStore = useAuthStore()
        axios.put(`${serverUrl}/recruitments/${id}/approve`, { status: true }, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${authStore.token}`,
            "x-access-token": authStore.token,
          ...
          self.recruitments = self.recruitments.map(item => item._id === id ? Object.assign(item, { status: true }) : item);
