    ...
    <div class="
      ...
      <div class="px-4 py-5 space-y-6 sm:p-6" v-if="user">
        ...
        <div class="rounded-lg">
          <img :src="imgUrlFor(serverUrl, user.profile?.cover, images.avatar)
      ...
      <div class="rounded-md flex-col justify-center items-center text-center p-6" v-else>
        <p class="text-sm">Search for a question?</p>
      ...
      <div class="px-4">
        <h3 class="text-sm text-gray-400 mb-4">Top questions</h3>
        <div class="flex items-center mb-4" v-for="(post, idx) in posts" :key="'top-post' + idx">
...
export default {
  ...
  async mounted() {
    this.posts = await this.getTopPostsData();
  },
  methods: {
    ...mapActions(usePostStore, ["getTopPostsData"]),
