    ...
    <div class="overflow-hidden bg-gray-50 pt-6 tabs-panel shadow-md border-r border-gray-100">
      <div class="px-4 py-5 space-y-6 sm:p-6" v-if="user">
        ...
        <div class="rounded-lg">
          <img :src="user.profile?.cover ? imgUrlFor(serverUrl, user.profile.cover) : 'https://learn.zoner.com/wp-content/uploads/2015/06/040mm.jpg?fidl=2019-06-mag-en'" alt="profile"
      ...
      <div class="px-4">
        ...
        <div class="flex items-center mb-4" v-for="(recruitment, idx) in recruitments" :key="idx">
          ...
          <div>
            ...
            <p class="text-sm ml-4 capitalize">{{truncate(recruitment.name, 24)}}</p>
          </div>
        </div>
      </div>
